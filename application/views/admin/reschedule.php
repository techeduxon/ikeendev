            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>Reschedule</h2>
                        <ul class="breadcrumb">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="header">
                                ...
                                <li style="list-style: none;"><a href="javascript:void(0);" class="btn btn-info pull-right" data-toggle="modal" data-target="#addmodal">Add Reschedule</a></li>
...
<div class="modal animated fadeIn" id="addmodaldet" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="title" id="defaultModalLabel">Add </h6>
            ...
            <div class="modal-body">
                <form class="form-group" id="addformdet"  method="post" enctype="multipart/form-data">
                <div class="row clearfix">

                      <div class="col-md-12 col-sm-12">
                        <div class="form-group">
                         <label for="name"> Content </label>
                            <textarea type="text" class="form-control" name="content" id="content"  autocomplete="off" ></textarea>

                        </div>
                    </div>
                </div>
                <input type="hidden" name="rca_id" id="rca_id" >
                <button type="submit" class="btn btn-primary">Add</button>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">CLOSE</button>
            </form>
            </div>
...
<script type="text/javascript" language="javascript" >
 $(function(){
        var dataTable = $('.datatable-buttt').DataTable({
           ...
           "order":[],
           "ajax":{
                url:"<?= base_url() . 'DataTable/getRescheduleList'; ?>",
                type:"POST",
     ...
     $(document).on('click','.delete',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Delete_reschedule' ?>',
                data:{id:id},
                success:function(data){
...
$(document).on('click', '.update', function(){
       ...
       swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
           $.ajax({
            type: "POST",
            url: "<?=base_url('fetch_single_reschedule')?>",
            data: {id:id},
    ...
    $(':input[type="submit"]').prop('disabled', true);
     ...
     $.ajax({
          type: "POST",
          url: "<?=base_url('update_reschedule')?>",
  ...
  $(document).on('click','.addDetails',function(){
    ...
    $.ajax({
            type: "POST",
            url: "<?=base_url('fetch_single_reschedule_det')?>",
            data: {id:id},
            success: function(response) {
                    ...
                    // $('#content').val(obj['content'])
                     $("#rca_id").val(obj['id']);
                     $('#addmodaldet').modal('show');
               }
             });

  });
  $(document).on('submit','#addformdet',function(){
          event.preventDefault();
          $(':input[type="submit"]').prop('disabled', false);
          ...
          $.ajax({
            url:'<?= base_url().'update_reschedule_det' ?>',
            type:'POST',
            data:$(this).serialize(),
            success:function(data){


              $('#addmodaldet').modal('hide');
