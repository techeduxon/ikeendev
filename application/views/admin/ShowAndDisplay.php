            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>Navigation List</h2>
                        <ul class="breadcrumb">
                           ...
                           <?= $this->session->flashdata('message') ?>
                            <div class="table-responsive">
                                <table class=" datatable-buttt table table-hover table-bordered  js-basic-example dataTable table-custom m-b-0">
                                    ...
                                    <tbody>
                                        <?php if(!empty($page)){ foreach($page as $val){ ?>
                                        <tr>
                                            <td><?= $val['page'] ?></td>
                                            <td>
                                            <?php if($val['show'] == '1'){ ?>
                                                <a href="<?= base_url().'Adminpanel/Adminpanel/changeStatus?id='.$val['id'].'&show=0' ?>" style="color: red">Hide</a>
                                            <?php }else{ ?>
                                                <a href="<?= base_url().'Adminpanel/Adminpanel/changeStatus?id='.$val['id'].'&show=1' ?>">Show</a>
                                            <?php } ?>
                                            </td>
                                        </tr>
                                        <?php } } ?>
