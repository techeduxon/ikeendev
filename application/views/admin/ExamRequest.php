            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>Exam Request</h2>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="<?= base_url().'Adminpanel/index'?>"><i class="icon-home"></i></a></li>
                            <li class="breadcrumb-item active">Exam Request</li>
                        </ul>
                    </div>
...
<script type="text/javascript" language="javascript" >
 $(function(){
        var dataTable = $('.datatable-buttt').DataTable({
           ...
           "order":[],
           "ajax":{
                url:"<?= base_url() . 'DataTable/getExamReqList'; ?>",
                type:"POST",
     ...
     $(document).on('click','.approve',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Adminpanel/Adminpanel/appoveExam' ?>',
                data:{id:id},
                success:function(data){
                  $('.datatable-buttt').DataTable().ajax.reload();
                  swal('Success','exam approve successfully','success');
                },
                // error: function (xhr, error, thrown) {
                //    window.location.reload();
                // },
