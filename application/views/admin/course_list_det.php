...
<script type="text/javascript" language="javascript" >
 $(function(){
        var dataTable = $('.datatable-buttt').DataTable({
           ...
           "order":[],
           "ajax":{
                url:"<?= base_url() . 'DataTable/getCourselistdetList?id='; ?>"+<?= $_GET['id'] ?>,
                type:"POST",
...
<script>
  $(document).ready(function(){
      $(document).on('submit','#addform',function(){
          ...
          $.ajax({
            url:'<?= base_url().'Add_course_list_det' ?>',
            type:'POST',
            data:$(this).serialize(),
     ...
     $(document).on('click','.delete',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Delete_course_list_det' ?>',
                data:{id:id},
                success:function(data){
...
$(document).on('click', '.update', function(){
       ...
       swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
           $.ajax({
            type: "POST",
            url: "<?=base_url('fetch_single_course_list_det')?>",
            data: {id:id},
    ...
    $(':input[type="submit"]').prop('disabled', true);
     ...
     $.ajax({
          type: "POST",
          url: "<?=base_url('update_course_list_det')?>",
          data:$(this).serialize(),
          success: function(data) {
            if (data == 1) {
