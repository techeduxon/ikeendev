            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a>Job Category Detail</h2>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="<?= base_url().'Adminpanel/index'?>"><i class="icon-home"></i></a></li>
                            <li class="breadcrumb-item active">Job Category Detail</li>
                        </ul>
                    </div>            
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="header">
                                ...
                                <li style="list-style: none;"><a href="javascript:void(0);" class="btn btn-info pull-right" data-toggle="modal" data-target="#addmodal">+Add Job Category Detail</a></li>
...
<script type="text/javascript" language="javascript" >  
 $(function(){ 
        var dataTable = $('.datatable-buttt').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?= base_url() . 'DataTable/getJobcategorydetList?id='; ?>"+"<?= $_GET['id'] ?>",  
                type:"POST",
           },  
           "columnDefs":[  
                {  
...
<script>
  $(document).ready(function(){
     $(document).on('submit','#addform',function(){ 
          ...
          $.ajax({ 
            url:'<?= base_url().'Adminpanel/Blog/Add_jobcategory_det' ?>',
            type:'POST',
            data:$(this).serialize(),
     ...
     $(document).on('click','.delete',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Adminpanel/Blog/Delete_jobcategory_det' ?>',
                data:{id:id},
                success:function(data){
                  $('.datatable-buttt').DataTable().ajax.reload();
...
$(document).on('click', '.update', function(){
       ...
       swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
           $.ajax({
            type: "POST",
            url: "<?= base_url('Adminpanel/Blog/fetch_single_jobcategory_det') ?>",
            data: {id:id},
    ...
    $(':input[type="submit"]').prop('disabled', true);
     $.ajax({
          type: "POST",
          url: "<?=base_url('Adminpanel/Blog/update_jobcategory_det')?>",
          data:$(this).serialize(),
          success: function(data) {
