            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a> Technologies</h2>
                        <ul class="breadcrumb">
                    ...
                    <div class="col-md-6 col-sm-12">
                        <div class="form-group">
                         <label for="comment">Comment </label>
                          <input type="text" class="form-control" name="comment"  placeholder="Enter Comment" autocomplete="off" >
                        </div>
                    </div>
                </div>
...
<script type="text/javascript" language="javascript" >
 $(function(){
        var dataTable = $('.datatable-buttt').DataTable({
           ...
           "order":[],
           "ajax":{
                url:"<?= base_url() . 'DataTable/getTechnologies' ?>",
                type:"POST",
           },
           "columnDefs":[
                {
...
<script>
  $(document).ready(function(){
     $(document).on('submit','#addform',function(){
          ...
          $.ajax({
            url:'<?= base_url().'Adminpanel/Adminpanel/addTechnology' ?>',
            type:'POST',
     ...
     $(document).on('click','.delete',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Adminpanel/Adminpanel/deleteTechnologies' ?>',
                data:{id:id},
                success:function(data){
                  $('.datatable-buttt').DataTable().ajax.reload();
