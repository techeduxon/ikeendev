                        ...
                        <div class="body">
                            <div class="teb-links">
                                <ul>
                                    ...
                                    <li><a href="javascript:void(0)" class="btn btn-outline-dark active" ><i class="la la-graduation-cap"></i> Review</a></li>
                    ...
                    <div class="col-md-4 col-sm-12">
                        <div class="form-group">
                         <label for="image">Image</label>
                            <input type="file" class="form-control" name="image" autocomplete="off" accept="image/*">
                        </div>
                    </div>
...
<script type="text/javascript" language="javascript" >
 $(function(){
        var dataTable = $('.datatable-buttt').DataTable({
           ...
           "order":[],
           "ajax":{
                url:"<?= base_url() . 'DataTable/getCourseReviewList?id='.$_GET['id']; ?>",
                type:"POST",
           },
           "columnDefs":[
                {
...
<script>
  $(document).ready(function(){
     $(document).on('submit','#addform',function(){
          ...
          $.ajax({
            url:'<?= base_url().'Adminpanel/Adminpanel/addCourseReview' ?>',
            type:'POST',
     ...
     $(document).on('click','.delete',function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
                type:'get',
                url:'<?= base_url().'Adminpanel/Adminpanel/deleteCourseReview' ?>',
                data:{id:id},
                success:function(data){
                  $('.datatable-buttt').DataTable().ajax.reload();
    ...
    $(':input[type="submit"]').prop('disabled', true);
     $.ajax({
          type: "POST",
          url: "<?= base_url().'Adminpanel/Adminpanel/updateCourseReview' ?>",
          data:new FormData(this),
          contentType:false,
          processData:false,
          success: function(data) {
